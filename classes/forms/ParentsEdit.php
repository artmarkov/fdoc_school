...
use main\eav\object\Parents;
use main\forms\core\DispMode as form_dispMode;

class ParentsEdit extends form_ObjEdit
{

    protected $timestamp;

    /**
     * ParentsEdit constructor.
     * @param $model
     * @param $url
     */
    public function __construct($obj, $url, $prefix = '')
    {
        ...
        $objAuth = new form_auth_Acl('form_ParentsEdit');
        parent::__construct('f', 'Сведения родителях', $objDS, $objAuth);
        $this->setRenderer(new form_render_Flight('ParentsEdit.phtml'));
        $this->setUrl($url);
        ...
        $fRelation = $this->addFieldset('form_core_Dynamic', 'relation', 'Сведения об учениках', $this->getDataSource()->inherit('relation'), new form_auth_Acl('public'));
        $fRelation->setRequireOneElement(true);
        $fRelation->addField('form_control_Smartselect', 'students_id', 'Ученик', ['type' => 'students', 'cssSize' => 'sm', 'submit' => 1, 'required' => 1]);
    ...
    public function save($force = false)
    {
        ...
        if ($addressStr) {
            ...
            if ($geoLookup) {
                try {
                    $o = \ObjectFactory::parents($this->getDataSource()->getObjId());

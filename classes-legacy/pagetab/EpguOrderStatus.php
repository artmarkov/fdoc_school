...
class pagetab_EpguOrderStatus extends pagetab_Abstract
{
    protected $order;
    protected $route;
    protected $readOnly;
    ...
    protected function setViewParams($view)
    {
        ...
        [$view->statusList, $view->statusPlaceHolderList, $view->statusCommentAllowedList, $view->statusFileAllowedList, $view->currentStatus] = $this->getStatusInfo();
        $view->documentList = $this->getDocumentList();
        $view->statusHist = $this->getHistory(); // История статусов
        $view->statusColor = Factory::get($this->order)->getStatusesInColor();
    ...
    protected function getStatusInfo()
    {
        ...
        foreach ($nextStatuses as $id => $meta) {
            ...
            if ($placeholder) {
                $placeholders[] = 'st_comment_placeholder[' . $id . ']="' . $placeholder . '";';
            }

            if ($ot->isStatusCommentAllowed($id)) {
                $commentFlags[] = $id;
            }
            if ($ot->isStatusFileAllowed($id)) {
                $fileFlags[] = $id;
            }
